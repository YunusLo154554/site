  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col">
      <header className="border-b border-[#1e1e2e] px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0a0a0f]/90 backdrop-blur-sm z-20">
        ...
        <span className="text-xs font-mono text-[#6b6b8a]">Genel Sohbet</span>
      </header>

      {/* Mesajlar */}
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-3xl w-full mx-auto">
        {loading ? (
          <p className="text-center text-[#3f3f5a] font-mono text-sm animate-pulse">Yükleniyor...</p>
        ) : messages.length === 0 ? (
          <p className="text-center text-[#3f3f5a] font-mono text-sm">Henüz mesaj yok. İlk mesajı gönder.</p>
        ) : (
                  ...
                  <div className={`max-w-[70%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-mono ${isAdmin ? 'text-[#7c3aed]' : 'text-[#6b6b8a]'}`}>
                        {msg.username}
                        {isAdmin && <span className="ml-1 text-[9px] bg-[#7c3aed]/20 border border-[#7c3aed]/30 px-1 rounded">admin</span>}
                      </span>
        ...
        <div className="max-w-3xl mx-auto">
          {user ? (
            ...
            <p className="text-center text-xs font-mono text-[#3f3f5a]">
              Mesaj göndermek için <Link to="/login" className="text-[#7c3aed] hover:underline">giriş yap</Link>
            </p>
          )}
        </div>
      </div>
