    ...
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4">
      <div className="w-full max-w-sm animate-fade-in">
        <div className="flex items-center gap-3 mb-10 justify-center">
          ...
          <span className="font-mono font-medium text-[#e2e2f0] tracking-tight text-lg">{t.siteTitle}</span>
          <button onClick={toggle}
            className="ml-2 text-[10px] font-mono px-2 py-1 rounded border border-[#1e1e2e] text-[#6b6b8a] hover:text-[#e2e2f0] transition-all">
            {lang === 'tr' ? 'EN' : 'TR'}
          </button>
        </div>
        ...
        <div className="bg-[#111118] border border-[#1e1e2e] rounded-2xl p-8">
          <div className="flex gap-1 mb-8 bg-[#0d0d14] rounded-lg p-1">
            {[['login', t.loginTitle], ['register', t.registerTitle]].map(([m, label]) => (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-mono text-[#6b6b8a] mb-2">{t.usernameLabel}</label>
            ...
            <div>
              <label className="block text-xs font-mono text-[#6b6b8a] mb-2">{t.passwordLabel}</label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password"
                  className="w-full bg-[#0d0d14] border border-[#1e1e2e] rounded-lg px-4 py-2.5 pr-16 text-sm text-[#e2e2f0] font-mono placeholder-[#3f3f5a] focus:outline-none focus:border-[#7c3aed]/60 transition-colors" />
                <button type="button" onClick={() => setShowPassword(p => !p)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#3f3f5a] hover:text-[#6b6b8a] transition-colors text-xs font-mono">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? t.waiting : mode === 'login' ? t.loginBtn : t.registerBtn}
            </button>
          </form>
        </div>
